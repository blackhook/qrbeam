...
网页应用位于 `apps/qrbeam-web`，包含 `/`、`/send/` 和 `/receive/` 三个入口。发送端会实时测量当前可见二维码区域、设备像素比和真实 QR 模块数，在 `1 / 5 / 8 / 11` 符号帧中选择最高可扫档位；只有模块至少为 `6` 物理像素才开始发送。底栏会显示符号数、FPS、模块数和实际像素密度；进入全屏通常可获得更高吞吐。稳定清单使用 M 纠错，数据使用档位对应的纠错级别；首次发送至少 3 秒清单，之后每 2 秒插入一轮清单。
