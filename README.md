...
网页应用位于 `apps/qrbeam-web`，包含 `/`、`/send/` 和 `/receive/` 三个入口。发送端会实时测量当前可见二维码区域、设备像素比和真实 QR 模块数，在 `1 / 5 / 8 / 11` 符号帧中选择最高可扫档位；只有模块至少为 `6` 物理像素才开始发送。每个二维码至少保持两个屏幕刷新周期，因此 60 Hz 显示器最高 30 FPS，120 Hz 显示器才允许 60 FPS。底栏会显示符号数、FPS、模块数和实际像素密度；进入全屏通常可获得更高吞吐。稳定清单使用 M 纠错，数据使用档位对应的纠错级别；首次发送至少 3 秒清单，之后每 2 秒插入一轮清单。

手机接收端使用相机识别二进制 QRBeam 帧，网页端从 ZXing 的 QR 字节段读取原始载荷并请求后置 1080p 相机，原生端启用高强度定位和降采样重试。恢复出的区块先写入 IndexedDB；只有全部区块的 CRC32C、总长度和完整文件 BLAKE3 都验证通过，才会显示“接收完成”并允许保存。页面刷新会恢复清单、完成区块和部分区块帧。
...
每帧 11 个 `256 B` 符号在 60 FPS 下的协议理论上限是 `168.96 kB/s`，但该档位要求 120 Hz 发送屏幕；普通 60 Hz 显示器的理论上限是 `84.48 kB/s`。手机相机、屏幕、距离和环境光下的实际吞吐尚未完成真机验证，不能把理论值当成实测值。
...
- 本机有 Apple 签名证书和描述文件时，生成 `dist/QRBeam-Alpha3-signed.ipa`。
- 缺少签名时，生成 `dist/QRBeam-Alpha3-unsigned.ipa`。无签名 IPA 不能直接安装，必须先用你自己的 Apple 证书重签名。

相机真实扫码、不同手机屏幕与电脑终端组合的稳定带宽目前没验证；这是 Alpha 3 最重要的实测项。
