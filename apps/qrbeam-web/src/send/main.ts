...
input.onchange = async () => { const file = input.files?.[0]; if (!file) return; if (file.size > MAX_FILE_BYTES) { status.textContent = "文件超过 100 MB 上限"; return; } await init(); status.textContent = "探测显示性能（约 2 秒）…"; fps = await probeFps(); const data = new Uint8Array(await file.arrayBuffer()); sender = new WebSender({ filename: file.name, mime_type: file.type, data, session_id: crypto.getRandomValues(new Uint8Array(16)), file_id: crypto.getRandomValues(new Uint32Array(1))[0] }); manifests = (sender.manifest_frames() as number[][]).map(frame => new Uint8Array(frame)); manifestCursor = warmup = sinceManifest = 0; clearInterval(timer); timer = window.setInterval(() => void tick(), 1000 / fps); void tick(); };
